...
_Generated 2026-05-12 13:27:20 — 30 checkpoint(s), 178 features._

## Diagnostics — model health overview

### ⚠ `trust_instability` · **warn** — 121/178 features are unstable across checkpoints

**What was observed.** More than half (68%) of features changed archetype between checkpoints (similarity-weighted stability < 0.5).
...
**What was observed.** Noise 15%, Catalyst 22%, Complex 30%.
...
**What was observed.** k=2 functional groups, silhouette=0.29, permutation p=0.000, bootstrap ARI=0.14, best group NC fraction=19%.  Abort triggered because: no group has >50% Noise Candidates (best=19%) ; bootstrap stability low (ARI=0.14<0.5).
...
**What was observed.** Final-checkpoint mean Volatility = 2.033e-06 ; median of earlier checkpoints = 2.793e-06 (ratio = 0.73×).
...
### ℹ `trust_keep_recommended` · **info** — 29 features are stably important across all checkpoints
...
|-----|------|-----|-----|-----|
| impact | 0.0114 | 0.0307 | 0.0002 | 0.2724 |
| volatility | 0.0000 | 0.0000 | 0.0000 | 0.0001 |
| nonlinearity | 0.0037 | 0.0110 | 0.0002 | 0.1219 |
| interaction | 0.0826 | 0.1238 | 0.0102 | 1.0595 |
...
|--|--|--|--|--|
| 1 | gwl | 0.2724 | 1.0595 | Catalyst |
| 2 | gwl_t-1 | 0.1989 | 0.7923 | Catalyst |
| 3 | gwl_t-2 | 0.1478 | 0.6094 | Catalyst |
| 4 | gwl_t-3 | 0.1094 | 0.4630 | Catalyst |
| 5 | rain | 0.0878 | 0.5266 | Catalyst |
| 6 | gwl_t-4 | 0.0777 | 0.3279 | Catalyst |
| 7 | stgH | 0.0625 | 0.2675 | Catalyst |
| 8 | gwl_t-5 | 0.0609 | 0.2372 | Catalyst |
| 9 | gwl_t-6 | 0.0483 | 0.2048 | Catalyst |
| 10 | rain_t-1 | 0.0464 | 0.3564 | Catalyst |
...
|--|--|--|--|
| Noise | 27 | 15.2% | low everywhere — candidate for pruning |
| Hidden Interactor | 5 | 2.8% | weak alone, strong via interactions |
| Catalyst | 40 | 22.5% | strong AND interacting — load-bearing |
| Nonlinear Driver | 11 | 6.2% | strong with curved relationship |
| Volatile Specialist | 9 | 5.1% | strong but context-dependent |
| Stable Contributor | 33 | 18.5% | moderate, reliable |
| Complex Driver | 53 | 29.8% | complex behaviour across all four axes |
...
|--|--|--|
| INVESTIGATE (unstable) | 121 | archetype flipped — role uncertain |
| MONITOR (borderline) | 28 | stability between 0.5 and 0.7 |
| CONFIDENTLY KEEP | 26 | stable + important — load-bearing |
| KEEP (stable) | 3 | stable but moderate importance |

**Investigate** (121): `gwl_t-24`, `gwl_t-23`, `gwl_t-22`, `gwl_t-21`, `gwl_t-19`, `gwl_t-18`, `gwl_t-17`, `gwl_t-16`, `gwl_t-15`, `gwl_t-12`
...
- **k** = 2 groups
- **silhouette** = 0.290  _(higher = tighter clusters; >0.5 is strong, >0.2 is moderate)_
- **permutation p-value** = 0.000  _(<0.05 means clusters aren't random)_
- **bootstrap ARI** = 0.142  _(≥0.5 means clustering is stable)_
- **best NC fraction** = 18.9%  _(needs >50% to prune)_

| Group | Size | NC % | Mean Impact | Recommendation |
|--|--|--|--|--|
| 0 | 141 | 14.2% | 0.0133 | KEEP — mostly useful |
| 1 | 37 | 18.9% | 0.0043 | KEEP — mostly useful |
