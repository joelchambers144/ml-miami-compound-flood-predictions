...
_Generated 2026-05-12 13:27:22 — 30 checkpoint(s), 181 features._

## Diagnostics — model health overview

### ⚠ `trust_instability` · **warn** — 137/181 features are unstable across checkpoints

**What was observed.** More than half (76%) of features changed archetype between checkpoints (similarity-weighted stability < 0.5).
...
**What was observed.** Noise 11%, Catalyst 22%, Complex 27%.

**Why it matters.** No archetype dominates — the model has a diverse mix of feature roles.

**What to do.** None.

### ℹ `co_sensitivity` · **info** — Co-Sensitivity refused to recommend any prune (5 groups found, but no prune-safe group)

**What was observed.** k=5 functional groups, silhouette=0.16, permutation p=0.000, bootstrap ARI=0.16, best group NC fraction=50%.  Abort triggered because: bootstrap stability low (ARI=0.16<0.5).
...
**What was observed.** Final-checkpoint mean Volatility = 3.385e-06 ; median of earlier checkpoints = 3.912e-06 (ratio = 0.87×).
...
### ℹ `trust_keep_recommended` · **info** — 26 features are stably important across all checkpoints
...
|-----|------|-----|-----|-----|
| impact | 0.0096 | 0.0204 | 0.0003 | 0.1320 |
| volatility | 0.0000 | 0.0000 | 0.0000 | 0.0001 |
| nonlinearity | 0.0054 | 0.0054 | 0.0007 | 0.0411 |
| interaction | 0.1776 | 0.1357 | 0.0509 | 0.8814 |
...
|--|--|--|--|--|
| 1 | gwl | 0.1320 | 0.8154 | Catalyst |
| 2 | gwl_t-1 | 0.1097 | 0.6603 | Catalyst |
| 3 | gwl_t-2 | 0.0897 | 0.6308 | Catalyst |
| 4 | rain | 0.0880 | 0.8814 | Catalyst |
| 5 | gwl_t-3 | 0.0850 | 0.5412 | Catalyst |
| 6 | gwl_t-4 | 0.0742 | 0.4558 | Catalyst |
| 7 | gwl_t-5 | 0.0661 | 0.3853 | Catalyst |
| 8 | rain_t-1 | 0.0629 | 0.7472 | Catalyst |
| 9 | rain_t-2 | 0.0627 | 0.6297 | Catalyst |
| 10 | gwl_t-6 | 0.0572 | 0.3272 | Catalyst |
...
|--|--|--|--|
| Noise | 20 | 11.0% | low everywhere — candidate for pruning |
| Hidden Interactor | 7 | 3.9% | weak alone, strong via interactions |
| Catalyst | 39 | 21.5% | strong AND interacting — load-bearing |
| Nonlinear Driver | 18 | 9.9% | strong with curved relationship |
| Volatile Specialist | 15 | 8.3% | strong but context-dependent |
| Stable Contributor | 34 | 18.8% | moderate, reliable |
| Complex Driver | 48 | 26.5% | complex behaviour across all four axes |
...
|--|--|--|
| INVESTIGATE (unstable) | 137 | archetype flipped — role uncertain |
| CONFIDENTLY KEEP | 26 | stable + important — load-bearing |
| MONITOR (borderline) | 18 | stability between 0.5 and 0.7 |

**Investigate** (137): `gwl_t-24`, `gwl_t-23`, `gwl_t-22`, `gwl_t-21`, `gwl_t-20`, `gwl_t-19`, `gwl_t-16`, `gwl_t-15`, `gwl_t-14`, `gwl_t-13`
...
- **k** = 5 groups
- **silhouette** = 0.158  _(higher = tighter clusters; >0.5 is strong, >0.2 is moderate)_
- **permutation p-value** = 0.000  _(<0.05 means clusters aren't random)_
- **bootstrap ARI** = 0.160  _(≥0.5 means clustering is stable)_
- **best NC fraction** = 50.0%  _(needs >50% to prune)_

| Group | Size | NC % | Mean Impact | Recommendation |
|--|--|--|--|--|
| 0 | 58 | 10.3% | 0.0215 | KEEP — mostly useful |
| 1 | 39 | 2.6% | 0.0049 | KEEP — mostly useful |
| 2 | 16 | 50.0% | 0.0029 | REVIEW — significant noise |
| 3 | 31 | 9.7% | 0.0035 | KEEP — mostly useful |
| 4 | 37 | 5.4% | 0.0038 | KEEP — mostly useful |
